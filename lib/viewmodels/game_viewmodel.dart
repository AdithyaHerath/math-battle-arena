...
class GameViewModel extends ChangeNotifier {
  ...
  void initializeSinglePlayer(String difficulty, String playerName) {
    _difficulty = difficulty;
    _myName = playerName;
    _opponentName = 'Clock';
    _playerId = 'single_player';
    _roomId = 'single_player_${DateTime.now().millisecondsSinceEpoch}';

    _questions = _gameService.generateQuestions(difficulty);
  ...
  void _startGameTimer() {
    _gameTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (_gameTimeLeft <= 0) {
        timer.cancel();
        if (_roomId.startsWith('single_player')) {
          _status = GameStatus.finished;
          _questionTimer?.cancel();
        } else {
  ...
  void _moveToNextQuestion() {
    ...
    if (_roomId.startsWith('single_player') &&
        _localQuestionIndex >= _questions.length) {
      _status = GameStatus.finished;
      _questionTimer?.cancel();
      _gameTimer?.cancel();
    } else if (mounted) {
